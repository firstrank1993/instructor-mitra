...
const CRITERIA = [
  { name: 'Safety consciousness', subs: [
    { name: 'Dress code', max: 2 }, { name: 'Use PPE', max: 5 }, { name: 'Apply/practice safety', max: 8 },
  ], total: 15 },
  ...
  { name: 'Attendance/ Punctuality', subs: [
    { name: 'Initiative', max: 3 }, { name: 'Accountability', max: 3 }, { name: 'Participative in work', max: 4 },
  ], total: 10 },
  { name: 'Ability to follow Manuals/ Written instructions', subs: [
    { name: 'Select right manual', max: 1 }, { name: 'Search for appropriate topic', max: 2 }, { name: 'Read & interpret the manual', max: 2 },
  ], total: 5 },
  { name: 'Application of Knowledge', subs: [
    { name: 'Plan the work', max: 4 }, { name: 'Select appropriate tools \n& equipment', max: 3 }, { name: 'Review the work', max: 3 },
  ], total: 10 },
  { name: 'Skills to handle tools & equipment', subs: [
    { name: 'Handle & use tools & \nequipment', max: 4 }, { name: 'Maintain safety in handling', max: 3 }, { name: 'Care & maintain', max: 3 },
  ], total: 10 },
  { name: 'Speed in doing work', subs: [
    { name: 'Properly sequence the work', max: 3 }, { name: 'Use appropriate technique', max: 5 }, { name: 'Review the work during execution', max: 2 },
  ], total: 10 },
  { name: 'Quality in workmanship', subs: [
    { name: 'Achieve work with high accuracy', max: 7 }, { name: 'Conform to requirement', max: 3 }, { name: 'Satisfy the purpose', max: 5 },
  ], total: 15 },
  { name: 'VIVA', subs: [
    { name: 'Response with clarity', max: 7 }, { name: 'Technical understanding', max: 5 }, { name: 'Conscious towards job role', max: 3 },
  ], total: 15 },
];
...
function setCell(ws, addr, value, opts = {}) {
  ...
  cell.value = value;
  cell.font = { name: 'Arial MT', size: opts.size || 10, bold: !!opts.bold, color: { argb: 'FF000000' } };
  cell.alignment = {
    horizontal: opts.h || 'center', vertical: 'middle',
    wrapText: opts.wrap !== false, textRotation: opts.rotate || 0,
  };
...
function safeSheetName(rollNumber, enrollmentNumber, usedNames) {
  ...
  while (usedNames.has(name)) {
    name = `${base}_${suffix}`.substring(0, 31);
    suffix++;
  }
  usedNames.add(name);
  return name;
...
export const generateFAR1Excel = async (reportData) => {
  ...
  for (const trainee of trainees) {
    ...
    for (const lo of sortedLOs) {
      ...
      setCell(ws, ws.getCell(currentRow, 2).address, loName, { bold: true, h: 'left', fill: 'FFAFEEEE', wrap: true });
      ws.mergeCells(currentRow, 33, currentRow, 39);
      setCell(ws, ws.getCell(currentRow, 33).address, `Average of LO${lo.loNumber}`, { bold: true, h: 'right', fill: 'FFAFEEEE' });
    ...
    setCell(ws, ws.getCell(currentRow, 2).address, 'Average of All LO', { bold: true, h: 'left', fill: 'FFD3D3D3' });
