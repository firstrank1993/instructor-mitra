...
async function runSeed() {
  console.log('🚀 Starting Instructor Mitra database seed...');
  console.log('================================================');

  try {
    // 1. System Config
    console.log('\n📋 Seeding system config...');
    await setDoc(doc(db, 'systemConfig', 'general'), {
    ...
    for (const criteria of defaultCriteria) {
      ...
      console.log(` ✅ ${criteria.name} (${criteria.maxMarks} marks)`);
    }
    console.log('✅ All 9 criteria seeded!');
    ...
    for (const trade of trades) {
      ...
      console.log(` ✅ ${trade.name} - ${trade.duration} year(s) - ID: ${ref.id}`);
    }
    console.log('✅ All trades seeded!');
    ...
    console.log(' ✅ 9 assessment criteria with sub-criteria');
    console.log(' ✅ 5 sample trades (you can add more from admin panel)');
    console.log('\nYou can now start Phase 4!');
